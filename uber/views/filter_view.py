...
def filter(request):
    ...
    if results.exists():
        ...
        total_faturamento = results.aggregate(Sum('faturamento'))[
            'faturamento__sum']
        media_faturamento = round(results.aggregate(
            Avg('faturamento'))['faturamento__avg'], 2)

        total_km_dirigido = round(results.aggregate(Sum('km_rodado'))[
            'km_rodado__sum'], 2)
        media_km_dirigido = round(results.aggregate(
            Avg('km_rodado'))['km_rodado__avg'], 2)

        total_gasto_comb = results.aggregate(Sum('gasto_com_comb'))[
            'gasto_com_comb__sum']
        media_gasto_comb = round(results.aggregate(
            Avg('gasto_com_comb'))['gasto_com_comb__avg'], 2)
        media_gasto_km = round(results.aggregate(
            Avg('gasto_por_km'))['gasto_por_km__avg'], 2)
        media_lucro_km = round(results.aggregate(
            Avg('ganho_por_km'))['ganho_por_km__avg'], 2)
        media_lucro_hora = round(results.aggregate(
        ...
        sql_horas_trab = ResultUber.objects.filter(
            owner=request.user, data_criacao__gte=start_date,
